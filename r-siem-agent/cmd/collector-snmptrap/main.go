...
import (
	...
	"r-siem-agent/internal/collector/snmptrap"
...
func main() {
	configPath := flag.String("config", "configs/collector-snmptrap.yaml", "Path to collector config")
	flag.Parse()
	...
	nc, err := nats.Connect(cfg.JetStream.URL, nats.Name("r-siem-collector-snmptrap"))
	if err != nil {
	...
	collector := snmptrap.New(snmptrap.Config{
