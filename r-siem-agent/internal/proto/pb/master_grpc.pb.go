...
const (
	Ingest_Stream_FullMethodName = "/master.v1.Ingest/Stream"
)
...
type IngestClient interface {
	Stream(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[Batch, Ack], error)
}
...
func (c *ingestClient) Stream(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[Batch, Ack], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &Ingest_ServiceDesc.Streams[0], Ingest_Stream_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
...
type Ingest_StreamClient = grpc.BidiStreamingClient[Batch, Ack]
...
type IngestServer interface {
	Stream(grpc.BidiStreamingServer[Batch, Ack]) error
...
func (UnimplementedIngestServer) Stream(grpc.BidiStreamingServer[Batch, Ack]) error {
...
func _Ingest_Stream_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(IngestServer).Stream(&grpc.GenericServerStream[Batch, Ack]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type Ingest_StreamServer = grpc.BidiStreamingServer[Batch, Ack]
