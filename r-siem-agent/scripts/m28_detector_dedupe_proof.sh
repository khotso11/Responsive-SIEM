...
baseline_dedup="$(last_line_num '"msg":"detect_dedup_hit"' "$LOG_DETECTOR")"
...
dedup_line="$(wait_new_line '"msg":"detect_dedup_hit"' "$LOG_DETECTOR" "$baseline_dedup")"
echo "$dedup_line"
...
echo "OK: dedupe hit and no new run for duplicate event_idem_key"
