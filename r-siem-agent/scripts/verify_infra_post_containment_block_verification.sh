...
RULE_ID="R-INFRA-POST-CONTAINMENT-BLOCK-VERIFY"
PLAYBOOK_ID="PB-INFRA-POST-CONTAINMENT-BLOCK-VERIFY-NOTIFY"
...
PROOF_JSON="${ART_DIR}/infra_post_containment_block_verification_proof.json"
...
host_tag="edge-fw-block-$(date +%H%M%S)"
ts_ms="$(date +%s%3N)"
msg="<13>Apr 07 20:45:00 ${host_tag} firewall action=drop src=10.44.1.10 dst=10.44.2.14 response_action_id=uiact_123 post_containment=blocked containment block verified ts=${ts_ms}"
...
echo "PASS: infrastructure post-containment block verification verified"
echo "INFRA_POST_CONTAINMENT_BLOCK_VERIFICATION_PROOF_JSON=${PROOF_JSON}"
