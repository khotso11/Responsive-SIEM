...
RULE_ID="R-INFRA-FIREWALL-DENY-BURST"
PLAYBOOK_ID="PB-INFRA-FIREWALL-DENY-BURST-NOTIFY"
...
PROOF_JSON="${ART_DIR}/infra_firewall_deny_burst_proof.json"
...
host_tag="edge-fw-verify-$(date +%H%M%S)"
count_sent=6
for i in $(seq 1 "$count_sent"); do
  ts_ms=$(( $(date +%s%3N) + i ))
  src_oct=$(( 40 + i ))
  msg="<13>Apr 07 20:00:00 ${host_tag} firewall policy denied src=10.60.0.${src_oct} dst=172.16.30.10 proto=tcp dpt=443 action=deny ts=${ts_ms}"
...
if (( published_delta < count_sent )); then
  fail_with_context "syslog collector published ${published_delta}, expected >= ${count_sent}"
fi
...
echo "PASS: infrastructure firewall deny burst verified"
echo "INFRA_FIREWALL_DENY_BURST_PROOF_JSON=${PROOF_JSON}"
