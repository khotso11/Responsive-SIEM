...
RULE_ID="R-INFRA-NETWORK-ADMIN-LOGIN"
PLAYBOOK_ID="PB-INFRA-NETWORK-ADMIN-LOGIN-NOTIFY"
...
PROOF_JSON="${ART_DIR}/infra_network_admin_login_proof.json"
...
host_tag="edge-rtr-verify-$(date +%H%M%S)"
ts_ms="$(date +%s%3N)"
msg="<14>Apr 07 20:00:00 ${host_tag} sshd[222]: Accepted password for admin from 10.70.0.10 port 22 ssh2 ts=${ts_ms}"
...
echo "PASS: infrastructure network admin login verified"
echo "INFRA_NETWORK_ADMIN_LOGIN_PROOF_JSON=${PROOF_JSON}"
